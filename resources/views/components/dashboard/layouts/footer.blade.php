 ...
 */
    ...
    function changeValVerification(input) {
    const value = input.value;
    const userId = input.dataset.userId; // if you want to use extra data
    const name = input.name;

    // alert("Selected value: " + userId);

    fetch('{{ url('admin/change-verification-status') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Content-Type': 'application/json'
