        ...
        <div class="content-wrapper">
            <x-dashboard.layouts.breadcrumb now="{{ __('dashboard.add_sponser') }}">
                <li class="breadcrumb-item"><a
                        href="{{ route('admin.sponsers.index') }}">{{ __('dashboard.sponsers_list') }}</a></li>
            ...
            <div class="col-12 mt-3">
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical" method="POST" action="{{ route('admin.sponsers.store') }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-3">
                                        <label class="w-100" for="days_number">{{ __('dashboard.days_number') }}
                                            <input type="number" id="days_number" class="form-control " max="365" min="1"
                                             name="days_number" 
                                                placeholder="{{ __('dashboard.days_number') }}"
                                                value="{{ old('days_number') }}" required />
                                            @error('days_number')
                                                <span style="font-size: 12px;" class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </label>
                                    </div>
                                    <div id="output" class="form-group col-3"></div>
...
@section('script')
    <script>
      $(document).ready(function(){
        const input = document.getElementById('days_number');
