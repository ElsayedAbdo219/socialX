<?php


return [
  "dashboard" => "لوحة التحكم",
  "login" => "تسجيل الدخول",
  'email or password or both are wrong' => 'البريد الالكتروني او كلمة المرور غير صحيحة',
  'hello' => 'مرحبا',
  'welcome_message' => 'مرحبا بك ',
  'Edit Profile' => 'تعديل الملف الشخصي',
  'Logout' => 'تسجيل الخروج',
  'name' => 'الاسم',
  'email' => 'البريد الالكتروني',
  'phone' => 'رقم الهاتف',
  'current_password' => 'كلمة المرور الحالية',
  'new_password' => 'كلمة المرور الجديدة',
  'password_confirmation' => 'تأكيد كلمة المرور',
  'show_password' => 'اظهار كلمة المرور',
  'edit' => 'تعديل',
  'edit_personal_data' => 'تعديل بيانات شخصية',
  'edit_password' => 'تعديل كلمة المرور',
  'App Notifications' => 'اشعارات التطبيق',
  'New' => 'جديدة',
  'read_all' => 'قراءة الكل',
  'personal-updated' => 'تم تحديث بياناتك الشخصية بنجاح',
  'goods_add' => 'اضافة بضاعة',
  'goods' => 'البضائع',
  'goods_list' => 'قائمة البضائع',
  'goods_add' => 'اضافة بضاعة',
  'no data available in table' => 'لا يوجد بيانات متاحة في الجدول',
  'records' => 'السجلات',
  'show' => 'اظهار',
  'search' => 'بحث',
  'Actions' => 'الإجراءات',
  'processing...' => 'جاري المعالجة...',
  'add' => 'اضافة',
  'submit' => 'ارسال',
  'created_goods' => 'تم اضافة البضاعة بنجاح',
  'no matching records found' => 'لم يتم العثور على سجلات مطابقة',
  'title edit' => 'تعديل',
  'title delete' => 'حذف',
  'delete' => 'حذف',
  'Are you sure ?' => 'هل أنت متأكد ؟',
  'No, cancel' => 'لا، إلغاء',
  'Yes, delete' => 'نعم احذف',
  'updated_goods' => 'تم تحديث بيانات البضاعة بنجاح',
  'deleted_goods' => 'تم حذف تلك البضاعة بنجاح',
  'settings' => 'الاعدادات',
  'app_settings' => 'اعدادات التطبيق',
  'Usage Terms' => 'شروط الاستخدام',
  'application privacy' => 'سياسة الخصوصية',
  'app about' => 'عن التطبيق',
  'contact ways' => ' طرق التواصل',
  'watsApp' => 'واتساب',
  'facebook' => 'فيسبوك',
  'snapchat' => 'سناب شات',
  'instagram' => 'انستجرام',
  'update' => 'تحديث',
  'Usage about' => 'عن الاستخدام',
  'phone Numbers' => 'ارقام الهاتف',
  'contact updated successfully' => 'تم تحديث بيانات الاتصال بنجاح',
  'Transactions' => 'المعاملات',
  'Transactions_list' => 'قائمة المعاملات',
  'traders' => 'تفاصيل التجار',
  'show_companies' => 'عرض الشركات',
  'show_suppliers' => 'عرض الموردين',
  'show_clients' => 'عرض العملاء',
  'companies' => 'الشركات',
  'phone' => 'الهاتف',
  'deposit_money' => 'المبلغ المدفوع',
  'payment_type' => 'نوع الدفع',
  'first_amount' => 'الدفعة الأولي',
  'duration' => 'المدة',
  'total' => 'الاجمالي',
  'add_WholeSale' => 'اضافة بيع جملة',
  'wassal' => 'وصال',
  'on_site' => 'علي ارضه',
  'cash' => 'كاش',
  'installment' => 'اجل',
  'created_whole_successfully' => 'تم اضافة بيع جملة بنجاح',
  'update_whole_successfully' => 'تم تحديث بيانات البيع بنجاح',
  'deleted_whole_successfully' => 'تم حذف بيع جملة بنجاح',
  'created_sectoral_selling_successfully' => 'تم اضافة بيع القطاعي بنجاح',
  'add_sectoral_selling' => 'اضافة بيع القطاعي',
  'checking_account' => 'حساب جاري',
  'ton' => 'طن',
  'shekara' => 'شكارة',
  'quantity' => 'الكمية',
  'define_unit' => ' حدد الوحدة',
  'unit_price' => 'سعر الوحدة',
  'payment_amount' => 'المبلغ المدفوع',
  'update_sectoralselling_successfully' => 'تم تحديث بيانات البيع القطاعي بنجاح',
  'deleted_whole_successfully' => 'تم حذف بيع القطاعي بنجاح',
  'payment_amout' => 'المبلغ المدفوع',
  'unit' => 'الوحدة',
  'phone Number' => 'رقم هاتف',
  'purchasing_from_company' => 'شراء من شركة',
  'purchasing_from_supplier' => 'شراء من مورد',
  'operations_purshasing_from_companies' => 'عمليات الشراء من الشركات',
  'operations_purshasing_from_suppliers' => 'عمليات الشراء من الموردين',
  'add_purshasing_from_company' => 'اضافة عملية شراء من شركة',
  'add_purshasing_from_supplier' => 'اضافة عملية شراء من مورد',
  'about app updated successfully' => 'تم تحديث المعلومات بنجاح',
  'terms-and-conditions updated successfully' => 'تم تحديث شروط الاستخدام بنجاح',
  'app-privacy updated successfully' => 'تم تحديث سياسة الخصوصية بنجاح',
  "employees" => "المستقلين",
  "show_employees" => "عرض المستقلين",
  "news" => "الاخبار",
  "show_news" => "عرض الاخبار",
  "posts" => "المنشورات",
  "show_posts" => "عرض المنشورات",
  "posts_list" => "عرض كل المنشورات",
  "company" => "الشركة",
  "file_name" => "الملف",
  "is_Active" => "الحالة",
  "companies_list" => "قائمة الشركات",
  "company" => "الشركة",
  "company" => "الشركة",
  "edit_company" => "تعديل بيانات الشركة",
  "employees_list" => "قائمة المستقلين",
  "add_employee" => "اضافة مستقل جديد",
  "edit_employee" => "تعديل بيانات المستقل",
  "news" => "الاخبار",
  "contentNews" => "المحتوي",
  "news_add" => "اضافة خبر جديد",
  "created_news" => "تمت  اضافة خبر جديد بنجاح",
  "updated_news" => "تم  تحديث الخبر  بنجاح",
  "news_list" => "قائمة الاخبار",
  "address" => "العنوان",
  "inactive" => "غير مفعل",
  "active" => "مفعل",
  "status" => "الحالة",
  "created_at" => "تاريخ الانشاء",
  "website" => "الموقع الالكتروني",
  "create" => "اضافة",
  "logo_image" => "صورة الشعار",
  "coverletter" => "صورة الغلاف",
  "item deleted successfully" => "تم الحذف بنجاح",
  "Complains" => "الشكاوي",
  "complains_list" => "قائمة الشكاوي",
  "updated_at" => "تاريخ التحديث",

  "fqas" => " الاسئلة الشائعة",
  "fqa_list" => "قائمة الاسئلة الشائعة",
  "fqas_list" => "قائمة الاسئلة الشائعة",
  "order" => "الترتيب",
  "question_ar" => "السؤال",
  "answer_ar" => "الاجابة",
  "show_fqa" => "عرض سؤال شائع",
  "edit_fqa" => "تعديل سؤال شائع",
  "add_fqa" => "اضافة سؤال شائع",
  "content" => "المحتوي",
  "followers" => "عدد المتابعين",
  "image" => "الصورة",
  "jobs" => "الوظائف",
  "show_jobs" => "عرض الوظائف",
  "advertises" => "الاعلانات",
  "show_advertises" => "عرض الاعلانات",
  "advertises_list" => "قائمة الاعلانات",
  "add_advertise" => "اضافة اعلان جديد",
  "edit_advertise" => "تعديل بيانات الاعلان",
  "job_list" => "قائمة الوظائف",
  "add_job" => "اضافة وظيفة جديدة",
  "edit_job" => "تعديل بيانات الوظيفة",
  "job" => "الوظيفة",
  'job_name' => 'اسم الوظيفة',
  "employee_types" => "انواع المستقلين",
  "job_period" => "مدة الوظيفة",
  "overview" => "نظرة عامة",
  "job_category" => "فئة الوظيفة",
  "job_description" => "وصف الوظيفة",
  "salary" => "الراتب",
  "salary_period" => "مدة الراتب",
  "experience" => "الخبرة",
  "work_level" => "مستوى العمل",
  "employee_type" => "نوع المستقل",
  "period" => "المدة",
  "is_published" => "يتم نشره فالساعة",

  "advertises" => "الاعلانات",
  "add_advertise" => "اضافة اعلان جديد",
  "edit_advertise" => "تعديل بيانات الاعلان",
  "advertise_list" => "قائمة الاعلانات",
  "main_page" => "الصفحة الرئيسية",
  "normal" => "عادي",
  "advertisement" => "اعلان",
  "edit_post" => "تعديل بيانات الطلب",
  'Suggestion_send_from' => " تم اضافة اقتراج جديد من خلال ",
  'Suggestion added successfully' => "تم اضافة الاقتراح بنجاح ",
  'categories' => " فئات المهارات",
  'show_categories' => " عرض فئات المهارات",
  'skills' => " المهارت",
  'show_skills' => " عرض المهارات",
  'categories_add' => 'اضافة الفئة',
  'created_Category' => 'تم انشاء الفئة بنجاح!',
  'categories_list' => 'قائمة الفئات',
  'category' => 'الفئة',
  'skill_add' => "اضافة مهارة",
  'welcome_to_dashboard' => 'مرحبا بك في لوحة التحكم',
  'number_of_jobs_this_Day' => 'وظايف (اليوم)',
  'number_of_companies' => 'عدد الشركات',
  'number_of_employees' => 'عدد الموظفين',
  'number_of_advertises_this_day' => ' اعلانات (اليوم) ',
  'jobs' => 'الوظائف',
  'all' => 'الكل',
  'view_all' => 'رؤية الكل',
  'details_of_companies' => 'تفاصيل الشركات',
  'details_of_employees' => 'تفاصيل الموظفين',
  'disactives' => 'الغير نشيطين',
  'actives' => 'النشيطين',
  'total' => 'الاجمالي',
  'active_request' => 'طلب تنشيط',
  'number_of_appliers' => 'عدد المتقدمين',
  'number_of_jobs' => 'عدد الوظائف',
  'our-vision' => 'رؤيتنا',
  'why-choose-anceega-for-seekers' => 'لماذا تختار Anceega للباحثين؟',
  'why-choose-anceega-for-business-and-freelancers' => ' لماذا تختار Anceega للأعمال والمستقلين؟   ',
  'key-features' => ' الميزات الرئيسية   ',
  'user-responsibilities' => 'مسؤليات المستخدم',
  'company-responsibilities' => 'مسؤليات الشركة',
  'platform-usage' => 'استخدام المنصة',
  'account-suspension-policy' => 'سياسة تعليق الحساب',
  'Help-Shape-Anceega' => 'المساعدة في تشكيل عن ثقة',
  'custom-suggestions' => 'اقتراحات مخصصة',
  'your-rights' => 'حقوقك',
  'information-collect' => 'معلومات مجمعة',
  'how-use-data' => 'كيفية استخدام البيانات ؟',
  'help-support' => 'المساعدة & الدعم',
  'advertise-Anceega' => 'اعلانات عن ثقة',
  'why-advertise-withAnceega' => ' لماذا الاعلانات مع عن ثقة   ',
  'how-advertise-work-for-companies' => '   كيف تعمل الاعلانات (للشركات)   ',
  'how-advertise-work-for-users' => '   كيف تعمل الاعلانات (للمستخدمين)   ',
  'skill_add' => 'اضافة مهارة',
  'news_add' => 'اضافة خبر',
  'reason' => 'السبب',
  'pending' => 'قيد الانتظار',
  'cancelled' => 'ملغي',
  'rejected' => 'مرفوض',
  'approved' => 'موافق عليه',
  'status' => 'الحالة',
  'reason_cancelled' => 'سبب الالغاء',
  'adsStatus' => 'حالة الاعلان',
  'sponsers' => 'الرعاة',
  'sponsers_list' => 'قائمة الرعاة',
  'add_sponser' => 'اضافة راعي جديد',
  'edit_sponser' => 'تعديل بيانات الراعي',
  'sponser' => 'الراعي',
  'days_number' => 'عدد الايام',
  'price' => 'السعر',
  'payment_status' => 'حالة الدفع',
  'image' => 'الصورة',
  'user_id' => 'المستخدم',
  'sponser_created_successfully' => 'تم انشاء الراعي بنجاح',
  'sponser_updated_successfully' => 'تم تحديث بيانات الراعي بنجاح',
  'sponser_deleted_successfully' => 'تم حذف الراعي بنجاح',
  'sponser_status' => 'حالة الراعي',
  'add_sponser' => 'اضافة راعي جديد',
  'edit_sponser' => 'تعديل بيانات الراعي',
  'show_sponsers' => 'عرض الرعاة',
  'promotions' => 'العروض الترويجية',
  'promotions_list' => 'قائمة العروض الترويجية',
  'add_promotion' => 'اضافة عرض ترويجي جديد',
  'edit_promotion' => 'تعديل بيانات العرض الترويجي',
  'promotion' => 'العرض الترويجي',
  'promotion_created_successfully' => 'تم انشاء العرض الترويجي بنجاح',
  'promotion_updated_successfully' => 'تم تحديث بيانات العرض الترويجي بنجاح',
  'promotion_deleted_successfully' => 'تم حذف العرض الترويجي بنجاح',
  'show_promotions' => 'عرض العروض الترويجية',
  'discount' => 'الخصم',
  'start_date' => 'تاريخ البدء',
  'end_date' => 'تاريخ الانتهاء',
  'active' => 'نشط',
  'inactive' => 'غير نشط',
  'en_lang_only' => 'اللغة الانجليزية فقط',
  'ar_lang_only' => 'اللغة العربية فقط',
  'promotion_status_updated' => 'تم تحديث حالة العرض الترويجي بنجاح',
  'promotion_status' => 'حالة العرض الترويجي',
  'promotion_name' => 'اسم العرض الترويجي',
  'promotion_discount' => 'نسبة الخصم',
  'promotion_start_date' => 'تاريخ بدء العرض الترويجي',
  'promotion_end_date' => 'تاريخ انتهاء العرض الترويجي',
  'promotion_is_active' => 'العرض الترويجي نشط',
  'promotion_is_inactive' => 'العرض الترويجي غير نشط',
  'Search' => 'بحث',
  'search_placeholder' => 'ابحث هنا...',
  'number_of_posts' => 'عدد المنشورات',
  'number_of_advertises' => 'عدد الاعلانات',
  'number_of_companies' => 'عدد الشركات',
  'number_of_employees' => 'عدد الموظفين',
  'add_users_to_this_promotion' => "اضافة مستخدمين لهذا الخصم",
  'promotion_name' => 'اسم العرض الترويجي',
  'validity' => 'صلاحية العرض',
  'start_date' => 'تاريخ البدء',
  'end_date' => 'تاريخ الانتهاء',
  'days_count' => 'عدد الأيام',
  'discount' => 'الخصم',
  'with_period' => 'مع فترة',
  'with_days_number' => 'مع عدد الأيام',
  'choose_the_promotion_validity' => 'اختر صلاحية العرض الترويجي',
  'period' => 'فترة',
  'title' => 'العنوان',
  'adsprice' => 'سعر الاعلان',
  'show_Ads_price' => 'عرض سعر الاعلانات',
  'currency' => 'العملة',
  'updated_AdsPrice' => 'تم تحديث سعر الاعلانات',
  'ads_price_add' => 'اضافة سعر اعلانات',
  'resolution_number' => 'جودة اعلانات العرض الترويجي',
  'video' => 'الفيديو',
  'choose_file_type' => 'اختار نوع الملف',
  'is_poll' => 'هل هو استطلاع رأي؟',
  'poll' => 'استطلاع رأي',
  'polling' => 'استطلاع رأي',
  'yes' => 'نعم',
  'no' => 'لا',
  'polling_question' => 'سؤال الاستطلاع',
  'No_Logo' => 'لا يوجد ',
  'No_Cover_Letter' => 'لا يوجد  ',
  'No_Image' => 'لا يوجد ',
  'No_Video' => 'لا يوجد ',
  'No_Ads_Price' => 'لا يوجد  ',
  'No_Ads_Price' => 'لا يوجد  ',
  'No_Personal_Photo' => 'لا يوجد  ',
  'seconds' => 'الثواني',
  'seconds_placeholder' => 'ادخل عدد الثواني',
  'time' => 'الوقت',
  'welcome_to' => 'مرحبا بك',
  'work_mode' => 'وضع العمل',
  'title_ar' => 'العنوان  - باللغة العربية',
  'contentNews_ar' => 'المحتوي  - باللغة العربية',
  'title_en' => 'العنوان  - باللغة الإنجليزية',
  'contentNews_en' => 'المحتوي  - باللغة الإنجليزية',
  'verfication_account' => 'هل الحساب موثق ؟' ,
  'resolution' => 'الدقة',
  'type' => 'النوع',
  'video' => 'فيديو',
  'image' => 'صورة',
  'ads_price' => 'سعر الاعلان',
  'ads_price_add' => 'اضافة سعر اعلان',
  'ads_price_edit' => 'تعديل سعر اعلان',
  'ads_price_list' => 'قائمة اسعار الاعلانات',
  'ads_price_created_successfully' => 'تم انشاء سعر الاعلان بنجاح',
  'ads_price_updated_successfully' => 'تم تحديث سعر الاعلان بنجاح',
  'image_price_already_exists' => 'سعر الاعلان للصورة موجود بالفعل بنفس العملة',
  'video_price_already_exists' => 'سعر الاعلان للفيديو موجود بالفعل بنفس العملة وبنفس الدقة',
  'choose_type' => 'اختر النوع',
  'choose_resolution' => 'اختر الدقة',
  'choose_currency' => 'اختر العملة',
  'choose_ads_price' => 'اختر سعر الاعلان',
  'ads_price' => 'سعر الاعلان',
  'appearence_count_for_time' => 'سعر الظهور للمرة الواحدة '  ,
  'our_vision' => 'رؤيتنا',
  'why_choose_anceega_for_seekers' => 'لماذا تختار Anceega للباحثين؟',
  'why_choose_anceega_for_business_and_freelancers' => 'لماذا تختار Anceega للأعمال والمستقلين؟',
  'key_features' => 'الميزات الرئيسية',
  'user_responsibilities' => 'مسؤوليات المستخدم',
  'company_responsibilities' => 'مسؤوليات الشركة',
  'platform_usage' => 'استخدام المنصة',
  'account_suspension_policy' => 'سياسة تعليق الحساب',
  'help_shape_anceega' => 'المساعدة في تشكيل Anceega',
  'custom_suggestions' => 'اقتراحات مخصصة',
  'your_rights' => 'حقوقك',
  'information_collect' => 'معلومات مجمعة',
  'how_use_data' => 'كيفية استخدام البيانات؟',
  'help_support' => 'المساعدة والدعم',
  'advertise_anceega' => 'اعلانات Anceega',
  'why_advertise_with_anceega' => 'لماذا الاعلان مع Anceega'  ,
  'how_advertise_work_for_companies' => 'كيف تعمل الاعلانات (للشركات)',
  'how_advertise_work_for_users' => 'كيف تعمل الاعلانات (للمستخدمين)'  ,
  'add_users_to_this_promotion' => 'اضافة مستخدمين لهذا العرض الترويجي',
  'add_new' => 'اضافة جديد',
  'show_data_in_arabic' => 'عرض البيانات باللغة العربية',
  'show_data_in_english' => 'عرض البيانات باللغة الإنجليزية',
  'show_data_in_arabic_and_english' => 'عرض البيانات باللغة العربية والإنجليزية',
  'show_data_in_arabic_and_english_and_video' => 'عرض البيانات باللغة العربية والإنجليزية والفيديو',
  'show_data_in_arabic_and_video' => 'عرض البيانات باللغة العربية والفيديو',
  'show_data_in_english_and_video' => 'عرض البيانات باللغة الإنجليزية والفيديو',
  'show_data_in_video' => 'عرض البيانات بالفيديو',
  'show_data_in_arabic_and_english_and_video' => 'عرض البيانات باللغة العربية والإنجليزية والفيديو',
  'show_data_in_arabic_and_english_and_video_and_image' => 'عرض البيانات باللغة العربية والإنجليزية والفيديو والصورة',
  'current_videos' => 'الفيديوهات الحالية',
  'current_images' => 'الصور الحالية',
  'current_ads_price' => 'سعر الاعلانات الحالي',
  'current_ads_prices' => 'اسعار الاعلانات الحالية',
  'how_advertise_work_for_users' => 'كيف تعمل الاعلانات (للمستخدمين)',
  'how_advertise_work_for_companies' => 'كيف تعمل الاعلانات (للشركات)'  ,
  'how_advertise_work_for_users' => 'كيف تعمل الاعلانات (للمستخدمين)'  ,
  'how_advertise_work_for_companies' => 'كيف تعمل الاعلانات (للشركات)'  ,
  'how_advertise_work_for_users' => 'كيف تعمل الاعلانات (للمستخدمين)',
  'advertise_Anceega' => 'اعلانات Anceega',
  'save' => 'حفظ',
  'title' => 'العنوان',
  'number_of_members' => 'اجمالي عدد الأعضاء',
  'ads_archieve' => 'أرشيف الاعلانات',
  'show_ads_archieve' => 'عرض أرشيف الاعلانات',
  'all' => 'الكل',
  'certain_countries' => 'دول معينة',
  'countries' => 'الدول'






];
